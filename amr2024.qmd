...
leaflet() %>%
  ...
  addPolylines(data = geojson_data, color = ~pal(Admin), popup = ~paste(Admin), opacity = 0.8)%>%
...
df_lottr <- read.csv("~/GitHub/amr_2024/data/preserveandmaintain.csv", check.names=FALSE, row.names=1) 
datatable(df_lottr,options = c(table_options(), list(pageLength = 11)))
```
## Bridge Conditions
```{r}
