...
hchart(df_gdp, "column", hcaes(x = Year, y = Total, group = County)) %>%
  ...
  hc_subtitle(text = '<a href="https://apps.bea.gov/itable/?ReqID=70&step=1#" target="_blank">Source: <b>Bureau of Economic Analysis</b></a>') %>%
  hc_xAxis(title = list(text = "Year")) %>%
  hc_tooltip(shared = TRUE, pointFormat = "<span style='color:{series.color}'>{series.name}</span>: <b>{point.y}</b><br/>") %>%
  hc_legend(enabled = TRUE) %>%
  hc_add_theme(hc_theme_smpl()) %>%
...
highchart() %>%
  ...
  hc_subtitle(text = '<a href="https://data.bls.gov/timeseries/LAUMT122726000000006?amp%253bdata_tool=XGtable&output_view=data&include_graphs=true" target="_blank">Source: <b>Bureau of Labor Statistics</b></a>', align = 'left', style = list(fontFamily = 'Roboto Condensed')) %>%
